...
def main():
    ...
    download_tickers = tickers + market_tickers
    ...
    save_csv(raw_data, raw_data_file)

    print(f"\nRaw data saved to: {raw_data_file}")
    print(f"Rows: {len(raw_data)}")
    print(f"Tradable tickers: {tickers}")
    print(f"Market feature tickers only: {market_tickers}")
